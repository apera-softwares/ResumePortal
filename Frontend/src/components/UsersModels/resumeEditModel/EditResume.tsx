...
function cleanInlineStyleString(styleStr: string | null): string | null {
...
function sanitizeResumeHtmlForEditor(html: string): string {
  if (!html) return "";
  let sanitized = html;
...
export default function EditResume({ candidate, onSave, isInline = false, onClose, initialMode, isPublicPage = false }: EditResumeProps) {
    ...
    const loadContent = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const cleanSource = sanitizeResumeHtmlForEditor(sourceHtml);
          setStyleHeader("");
          setPreviewHtml(cleanSource);
          setRawHtml(cleanSource);
          setOriginalParsedHtml(data.resumeText ? sanitizeResumeHtmlForEditor(data.resumeText) : cleanSource);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const cleanedFinal = sanitizeResumeHtmlForEditor(finalHtmlContent);
      setStyleHeader("");
      setPreviewHtml(cleanedFinal);
      setRawHtml(cleanedFinal);
      setOriginalParsedHtml(cleanedFinal);
      setEditorKey(prev => prev + 1);
