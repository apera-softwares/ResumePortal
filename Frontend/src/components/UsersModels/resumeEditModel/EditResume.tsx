...
export default function EditResume({ candidate, onSave, isInline = false, onClose, initialMode, isPublicPage = false }: EditResumeProps) {
  ...
  const handleReparseResume = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/candidates/${candidate.id}/reparse-resume`, {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        ...
        const html = data.editedHtml || data.resumeText || "";
        setRawHtml(html);
        setPreviewHtml(html);
        setOriginalParsedHtml(data.resumeText || "");
      } else {
        alert("Failed to re-parse resume file.");
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (initialMode) {
      ...
      setIsEditMode(edit);
      if (!edit) {
        setViewMode((prev) => (prev === "edit" ? "preview" : prev));
      } else {
        setViewMode("edit");
      }
    }
    ...
    const loadContent = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // If editedHtml is present (user has saved edits), default viewMode to "preview" (Edited Resume)
          // so that View Resume displays the edited resume instead of the original unedited file!
          if (initialMode === "original" || initialMode === "review" || !initialMode) {
            setViewMode(data.editedHtml ? "preview" : "review");
  ...
  const renderResumeSurface = () => {
    ...
    if (viewMode === "preview" || !isEditMode) {
      const displayHtml = previewHtml || rawHtml;
      if (!displayHtml) {
        return (
          <div className="h-full overflow-hidden rounded-2xl border border-gray-200 bg-gray-100 shadow-inner dark:border-gray-800 dark:bg-gray-950">
            {renderOriginalResumeContent()}
      ...
      return (
        <div className="h-full overflow-hidden rounded-2xl border border-gray-200 bg-gray-100 shadow-inner dark:border-gray-800 dark:bg-gray-950">
          <iframe
            key={`${candidate.id}-${displayHtml.length}`}
    ...
    if (candidateResumePdf || candidateResume) {
      return (
        <div className="flex h-full w-full flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-8 text-center dark:border-gray-700 dark:bg-gray-900">
          ...
          <h3 className="mb-2 text-lg font-bold text-gray-900 dark:text-white">
            Resume PDF/Word Document Detected
          </h3>
          <p className="mb-6 max-w-md text-sm text-gray-500 dark:text-gray-400">
            An original resume file ({candidateResume || "Document"}) is available. Click below to parse the document into the visual interactive HTML editor.
          </p>
          <button
            onClick={handleReparseResume}
            disabled={isReparsing}
            className="inline-flex items-center gap-2 rounded-xl bg-purple-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-purple-500/25 transition-all hover:bg-purple-700 active:scale-95 disabled:opacity-50"
          >
            {isReparsing ? (
              <>
                ...
                <span>Parsing & Loading into Editor...</span>
              </>
            ) : (
              <>
                ...
                <span>Parse Resume to HTML Editor</span>
              </>
            )}
          </button>
        </div>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedCandidate = await response.json();
      const savedEditedHtml = updatedCandidate.editedHtml || fullHtml;
